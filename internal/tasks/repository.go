...
type Repository interface {
	...
	ListByProject(projectID string, status string, assignee string) ([]Task, error)
...
func (r *postgresRepository) Create(t *Task) error {
	...
	`
	_, err := r.db.Exec(context.Background(), query, t.ID, t.Title, t.Description, t.Status, t.Priority, t.ProjectID, t.AssigneeID, t.DueDate, t.CreatedAt, t.UpdatedAt)
	return err
}

func (r *postgresRepository) GetByID(id string) (*Task, error) {
...
func (r *postgresRepository) ListByProject(projectID string, status string, assignee string) ([]Task, error) {
...
func (r *postgresRepository) Update(t *Task) error {
	...
	_, err := r.db.Exec(context.Background(), query, t.Title, t.Description, t.Status, t.Priority, t.AssigneeID, t.DueDate, t.UpdatedAt, t.ID)
	return err
}
...
func (r *postgresRepository) GetProjectOwner(projectID string) (string, error) {
	var ownerID string
	query := `SELECT owner_id FROM projects WHERE id = $1`
	err := r.db.QueryRow(context.Background(), query, projectID).Scan(&ownerID)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return "", ErrNotFound
		}
